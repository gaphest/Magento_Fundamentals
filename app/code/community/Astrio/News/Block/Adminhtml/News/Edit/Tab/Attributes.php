 ...
 */
class Astrio_News_Block_Adminhtml_News_Edit_Tab_Attributes extends Mage_Adminhtml_Block_Catalog_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        if ($group) {
            ...
            $fieldset = $form->addFieldset('group_fields' . $group->getId(), array(
                'legend' => Mage::helper('astrio_news')->__($group->getAttributeGroupName()),
                'class' => 'fieldset-wide'
            ));
            ...
            // Set default attribute values for new news
            ...
            if ($this->_getEntity()->hasLockedAttributes()) {
                foreach ($this->_getEntity()->getLockedAttributes() as $attribute) {
    ...
    protected function _getAdditionalElementTypes()
    {
        $result = array(
            ...
            'image'    => Mage::getConfig()->getBlockClassName('astrio_news/adminhtml_news_helper_image'),
        );

        $response = new Varien_Object();
        $response->setTypes(array());
        Mage::dispatchEvent('adminhtml_astrio_news_edit_element_types', array('response' => $response));
